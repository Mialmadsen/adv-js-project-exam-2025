        ...
        <div class="bg-on-brand-text/70 w-full max-w-xl rounded-md shadow-md backdrop-blur-[2px]">
          ...
          <div class="space-y-4 px-5 py-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
              ...
              <span
                class="font-title ml-auto inline-block rounded-md px-2 py-1 text-xs"
                :class="
                  user?.emailVerified ? 'bg-brand/20 text-brand/80' : 'bg-danger/20 text-danger/80'
                "
              >
            ...
            <div class="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                <span class="font-title text-text">Size:</span> {{ user?.shirtSize || '—' }}
              </div>
            </div>
            ...
            <div>
              ...
              <div v-else-if="!registrations?.length" class="text-text/70 text-sm">
                No registrations.
              </div>
                ...
                >
                  <div class="flex min-w-0 items-center gap-2">
                    ...
                    <span
                      ...
                      class="font-title bg-brand/20 text-text/90 inline-flex shrink-0 items-center justify-center rounded-md px-2 py-0.5 text-[11px]"
                      :title="`Bib #${r.bib}`"
                    >
                      #{{ r.bib }}
                    </span>
                  ...
                  <span class="shrink-0 opacity-70">{{ r.status || '—' }}</span>
                </li>
              </ul>
            </div>
          </div>
          ...
          <!-- footer actions -->
          <div class="border-brand/10 flex items-center justify-between border-t px-5 py-4">
            <BaseButton variant="outline" size="sm" @click="emit('close')"> Close </BaseButton>

            <BaseButton variant="solid" size="sm" @click="emit('contact', user)">
              Contact user
            </BaseButton>
          </div>
        </div>
