...
import { downloadParticipantsCsv, downloadParticipantsJson } from '@/utils/exporters'
...
const pillVariant = (r) => ((!r && !race.value) || race.value === r ? 'solid' : 'outline')
...
<template>
  <section class="p-4 sm:p-6">
    <header class="mb-4 sm:mb-6">
      <h1 class="text-accent text-2xl font-semibold">{{ title }}</h1>
      ...
      <div class="mt-3 flex flex-wrap items-center gap-2">
        ...
        <BaseButton :variant="pillVariant('light')" size="sm" @click="go('light')"
          >LIGHT</BaseButton
        >
    ...
    <!-- admin modal -->
    <ParticipantDetailsModal :open="modalOpen" :participant="selected" @close="modalOpen = false" />
  </section>
</template>
